...
$\qquad$ In microbiome data, we usually consider the abundance of each sequence to be a variable and each sample to be a different observation, so calculating all pairwise covariances manually would be a big pain. Instead, we can calculate a covariance matrix $\Sigma$ using the linear algebra definition $\Sigma=X'X$, where $X$ is our data matrix (typically rows are observations, columns are variables), and $'$ denotes the transpose of the matrix (flip so rows are columns and columns are rows). Already this matrix can tell us a lot about the relationships between variables amongst our samples, but it's very hard to read especially for microbiomes with a large number of sequences. So we want to summarize this information in a way that is visualizable, which is where statistical ordination comes in.
$\qquad$ The goal of PCA is to describe this large covariance matrix $\Sigma$ using a fewer number of uncorrelated (also called orthogonal) variables which are some linear function of the variables we actually observed. [Modern Statistics for Modern Biology has an entire chapter about this which we should refer interested people to]. In order to do this, we calculate something called the eigenvalue decomposition of the covariance matrix. Basically what that means is to change reorganize $\Sigma$ as the multiplication of (1) a matrix, u, which tells you what values of those new variables each sample takes on; (2) a matrix, D, which tells you the weights of each of these new uncorrelated variables in capturing the information in original matrix $\Sigma$; (3) another matrix, v, which tells you what the combination of observed variables is for each axis. That allows us select variables with the highest associated weights and display those in a visualizable number of dimensions (~1-3). The weight of each of those variables is expressed by a number called and eigenvalue, which is proportional to the percent of variance explained by those axes. Because these variables are linear functions of the original observed variables, they are highly susceptible to problems in the structure of those original variables. For example, if the value of one variable tends to be much higher than the other variables, it disproportionately influences the PCA, which is why scaling the data to a simiar magnitude is often recommended. 
$\qquad$ So what else can we do for our microbiome data if regular PCA doesn't tell us what we want? PCoA does the same procedure, but instead of using the actual values of observed variables, we use distances generated between the samples using some metric. (need more details)
$\qquad$ What if I have a variable that I definitely want to enforce as being one of my axes? That's where CCA helps, it allows you to construct a PCA model but where the linear combination of variables that comprises one of the axes is prescribed by you. (need more details)
...
We just did a lot of work involving maneuvering around complicated covariance structure in our data and had to make judgment calls about a transformations and look at scree plots. What if I don't want ot do that? There is a method for you! Multidimensional Scaling allows us to view relationships between samples in a small number of dimensions, but does not rely on covariance matrix eigendecomposition. This is an optimization method, which means that there is some stated objective function at the beginning of the problem, and then the computer will do something fancy that isn't critical to know all of the details about to attempt to arrive at a solution which either maximizes or minimizes that objective function. In the case of (N)MDS, the optimization problem has everything to do with distance matrices. A distance matrix is the collection of all pairwise distances between samples as defined by a distance metric (eg, euclidean distance). The goal of the optimization problem is to generate a set of coordinates for each of your samples, which when you calculate the distance matrix using those coordinates, the distance matrix is (as close as possible to) identical to the original distance matrix using the observed variables.
This method has some advantages and disadvantages. 
Advantages: 
If your data have weird covariance structure that somehow would mess up an SVD-based method, this method is not so affected.
You enforce in advance how many dimensions you want to express the data in, so you never run into a situation with highly multicollinear data where doing SVD may explain the variance equally distributed amongst many axes.
Disadvantages:
This method is not guaranteed a unique solution (so make sure you seed your random seed before you run an MDS algorithm to reproduce!)
The output coordinates do not provide any additional summarizing information about the data apart from their pairwise distances, so the coordinates are a poor choice for further modeling/interpretation. Along these lines, if your data have any outliers, they will strongly influence the entire NMDS, and may make points that are pretty far apart in distance space look closer in NMDS coordinate space by comparison to their mutual distance from the outlier. 
The method is very sensitive to which distance metric you use, so choose carefully! Along these lines, although Bray-Curtis dissimilarity is extensively used in ordinations (both PCoA and NMDS) in microbial ecology, and is very interpretable, this index is not recommended for statistical ordination. The primary reason being that Bray-Curtis does not maintain the $\textit{Triangle Inequality}$, a mathematical principle which essentially ensures that the shortest distance between any two points is a straight line. This is important, because unless it is true visualizing multiple pairwise distances at once will be visually misleading, as points visually further apart from each other may actually not have more distance than points which are more neighboring. 
```{r Implementing NMDS}
...
index_frame %>%
  ...
  xlab('Number Clusters')
```
## How do I interpret these indices?
The goal of the problem - We want to cluster which OTUs have similar patterns in terms of their temporal dynamics.
Question: I don't know how many clusters I should partition my OTUs into, and I also don't know which algorithm to use to do the clustering?
Solution: Decide what algorithms you want to try (we are trying hierarchical and k-meds), then cluster your OTUs into different numbers of clusters (we are trying 2,3,..,10). Now we compare all of these runs (total is 9 cluster numbers*2 algorithms= 18 runs) to see which one is most parsimonious with the data.
Question: How do I know which run is the most parsimonious with the data?
Solution: We can compare a selection of indices which summarize the quality of any particular clustering of some data. The indices we're selecting in this problem are (1) SSE (sum of squared errors), (2) CH (the calinski-harabasz index), (3) silhouette profiles. What are those?
SSE: This is the same as squared errors in lots of other modeling applications (such as regression). The smaller this value is, the better, but also this index automatically becomes smaller as the number of clusters increases, so we use a heuristic called looking for the "elbow" in SSE across numbers of clusters. (more details)
CH: This index is related to the ratio of between cluster differences to within cluster differences, so the higher it is, the more similar the time series of OTUs within clusters are relative to those OTUs in other clusters. We want a high value for the CH index.
Silhouette Width: This index is unique compared to the other two because it is not a single number summary of the whole run, but rather a value calculated for every OTU that generates a profile. For each OTU, the silhouette width is defined as the difference between that signal's distance to  its nearest out-of-cluster OTU and its furthest in-cluster OTU dived by the larger of those. Therefore, if a silhouette width is negative, that means there is some other OTU that is in a different cluster that is more similar to that OTU than another OTU in its cluster (ie, clusters are diffuse). The closer that value is to 1 the more confidently that OTU belongs in its assigned cluster. This can be summarized by averaging over all OTUs for a general sense of whether the clusters are overall very strong or weaker. 
